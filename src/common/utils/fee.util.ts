...
export async function calculateFundingFee(
  ...
  try {
    ...
    if (configService) {
      try {
        fundingFee = normalizeFeePercentage(await configService.getConfig<Decimal>('ADMIN_FUNDING_FEE', FALLBACK_ADMIN_FUNDING_FEE));
        fundingFee100KAbove = normalizeFeePercentage(await configService.getConfig<Decimal>('ADMIN_FUNDING_FEE_100KABOVE', FALLBACK_ADMIN_FUNDING_FEE_100KABOVE));
      ...
      fundingFee = envFundingFee 
        ? normalizeFeePercentage(envFundingFee)
        : FALLBACK_ADMIN_FUNDING_FEE;
    ...
    const feePercentage = grossAmount.lte(fundingThreshold)
      ? fundingFee
      : fundingFee100KAbove;
...
export async function calculatePayoutFee(
  ...
  try {
    ...
    if (configService) {
      try {
        payoutFee = normalizeFeePercentage(await configService.getConfig<Decimal>('ADMIN_PAYOUT_FEE', FALLBACK_ADMIN_PAYOUT_FEE));
      } catch (error) {
      ...
      payoutFee = envPayoutFee
        ? normalizeFeePercentage(envPayoutFee)
        : FALLBACK_ADMIN_PAYOUT_FEE;
    }
